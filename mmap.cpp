...
bool mmap::inject() {
	...
	*/
	...
	uint64_t iat_function_ptr{ imports["QueryPerformanceCounter"] };
	if (!iat_function_ptr) { 
...
void mmap::solve_imports(uint8_t *base, IMAGE_NT_HEADERS *nt_header, IMAGE_IMPORT_DESCRIPTOR *import_descriptor) {
	...
	while ((module = (char *)get_ptr_from_rva((DWORD64)(import_descriptor->Name), nt_header, (PBYTE)base))) {
		...
		//if (!proc->get_module_base(module)) {
				//proc->load_library(module);
		//}
...
bool mmap::parse_imports() {
	auto base{ proc->get_module_base("DiscordHook64.dll") };
	std::cout << "\nbase for dischook.dll: 0x" << std::hex << base << "\n";
